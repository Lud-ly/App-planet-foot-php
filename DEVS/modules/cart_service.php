...
Class Cart_service extends Initialize	{
   ...
   * 
   * @param string|int $userId The user ID
   * @param string|int $promoReference The promo ID
   * 
   * @return array|null
   */
  public function cancelUserPromoReferenceUse($userId, $promoReference) {
		$sqlPath = $this->GLOBALS_INI["PATH_HOME"] . $this->GLOBALS_INI["PATH_MODEL"] . "cart__delete_user_unvalidated_promo.sql";
		return $this->oBdd->treatDatas($sqlPath, [
				'userId' => $userId,
				'promoReference' => $promoReference
		]);
  }
   ...
   */
  public function getUsedPromoReferenceByUserCount($userId, string $promo_code) {
