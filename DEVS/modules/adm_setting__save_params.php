...
class Adm_setting__save_params	{
    ...
    private function main() {
        ...
        $icParams = new Input_control(
            Adm_setting::$allowedParamsConfig,
            'params'
        );
        
        $icParams->checkAll($_POST['json']);

        $isValid = $icParams->isValid();

        if ($isValid) {
            // secured data and save it into database
            $objet_service->saveParams( $icParams->getData(true) );
        }

        // send the result to JS
        send_json_to_JS( ['is_valid' => $isValid] );
    }
