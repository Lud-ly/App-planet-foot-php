...
Class Adm_setting__service extends Initialize	{
     ...
     * 
     * @param array $jData The data to save in database
     * 
     * @return array
     */
    public function saveParams(array $jData) {
        $aResult = [];
        ...
        foreach ($jData as $paramName => $paramValue) {
            $paramId = $this->getParamId($paramName, $_SESSION['centerId']);
            $replacements = [
                'centerId' =>       $_SESSION['centerId'],
                'paramName' =>      $paramName,
